...
const before = await page.evaluate(
  () =>
    performance.getEntriesByType('resource').filter((r) => r.name.includes('world-tuas.glb'))
      .length,
);
...
await page.waitForFunction(
  () => document.querySelector('.world-canvas canvas')?.dataset.modelLoaded === 'tuas',
);
const storyReadyMs = Math.round(performance.now() - enter);
await page.waitForTimeout(1800);
const high = await canvas.evaluate((c) => ({ ...c.dataset }));
await page.screenshot({ path: 'docs/qa/tuas/present.png' });
for (let i = 2; i < 8; i++) {
  await page.locator('.tuas-journey .dtss-stop-list button').nth(i).click();
  await page.waitForTimeout(1800);
  await page.screenshot({ path: `docs/qa/tuas/stop-${i + 1}.png` });
}
await page.locator('.tuas-journey .dtss-stop-list button').nth(4).click();
...
await page.locator('.tuas-journey .dtss-stop-list button').nth(0).click();
...
const result = {
  ...
  usefulTextMs,
  storyReadyMs,
  tuasRequestsBeforeEntry: before,
...
writeFileSync('docs/qa/tuas/performance.json', JSON.stringify(result, null, 2) + '\n');
