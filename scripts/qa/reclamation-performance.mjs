...
const before = await page.evaluate(
  () =>
    performance.getEntriesByType('resource').filter((r) => r.name.includes('world-reclamation.glb'))
      .length,
);
...
await page.screenshot({ path: 'docs/qa/reclamation/present.png' });
for (let i = 2; i < 9; i++) {
  await page.locator('.reclamation-journey .dtss-stop-list button').nth(i).click();
  await page.waitForTimeout(1800);
  if ([2, 3, 4, 5, 6].includes(i))
    await page.getByRole('slider', { name: 'Tekong mechanism progress' }).fill('65');
  await page.screenshot({ path: `docs/qa/reclamation/stop-${i + 1}.png` });
}
await page.locator('.reclamation-journey .dtss-stop-list button').nth(5).click();
...
await page.locator('.reclamation-journey .dtss-stop-list button').nth(0).click();
await page.getByRole('checkbox', { name: 'Lower graphics load' }).check();
...
const result = {
  ...
  usefulTextMs,
  storyReadyMs,
  reclamationRequestsBeforeEntry: before,
...
writeFileSync('docs/qa/reclamation/performance.json', JSON.stringify(result, null, 2) + '\n');
