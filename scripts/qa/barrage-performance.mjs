...
const before = await page.evaluate(
  () =>
    performance.getEntriesByType('resource').filter((r) => r.name.includes('world-barrage.glb'))
      .length,
);
...
await page.screenshot({ path: 'docs/qa/barrage/present.png' });
for (let i = 2; i < 9; i++) {
  await page.locator('.barrage-journey .dtss-stop-list button').nth(i).click();
  await page.waitForTimeout(1800);
  if ([2, 3, 4, 5, 6].includes(i))
    await page.getByRole('slider', { name: 'Barrage mechanism progress' }).fill('65');
  await page.screenshot({ path: `docs/qa/barrage/stop-${i + 1}.png` });
}
await page.locator('.barrage-journey .dtss-stop-list button').nth(5).click();
...
await page.locator('.barrage-journey .dtss-stop-list button').nth(0).click();
...
const result = {
  ...
  usefulTextMs,
  storyReadyMs,
  barrageRequestsBeforeEntry: before,
...
writeFileSync('docs/qa/barrage/performance.json', JSON.stringify(result, null, 2) + '\n');
